...
\begin{center}
  \begin{tabular}{c|c||c|c|c}
    X & Class & $ \hat{\mu} $ & $ x - \hat{\mu} $ & $ \sum{(x - \hat{\mu})^{2}} $\\
    \hline
    \hline
    0 & A & 3.5 & & \\
    1 & A &     & & \\
...
$$ \hat{\mu_{1}} = \frac{\sum{x} }{n_{1} } = \frac{0+1+2+1+3}{5} = \frac{7}{5} = 3.5 $$
$$ \hat{\mu_{2}} = \frac{\sum{x} }{n_{2} } = \frac{1+2+3+4}{4} = \frac{10}{4} = 2.5 $$
$$ \hat{\mu_{3}} = \frac{\sum{x} }{n_{2} } = \frac{4+6+6+6}{4} = \frac{28}{4} = 7 $$

Compute a common variance $ \hat{\sigma}^{2} = \frac{1}{n-K} \sum \sum{(x - \hat{\mu})^{2}} $ \\

Compute the class membership probabilities $ \pi_{k} = \frac{n_{k} }{n}$
